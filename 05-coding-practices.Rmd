...
Sometimes, we'd like to apply the same wrapper function to a matrix or a list of lists where each row of the matrix or each list contains a different combination of interests. Then, we can consider using `mapply()` and `map()`, both of which are a part of the base R packages.

Assume we have a matrix to be processed:

```
# Convert matrix to list of lists, where each row is a list
mat_list <- as.list(matrix)

# Use mapply() to apply a function to each list
mapply(FUN = func, ... = mat_list)

# Use map() to apply a function to each list
map(.x = mat_list, .f = func)
````

As the syntax above shows, **`mapply()` and `map()` cannot directly process a matrix**, so we need to transform it into a list of lists first.

Please note that if your input lists are of different lengths, these two functions probably will not give the preferable results. If using `map()`, the function is only applied to the elements up to the length of the shortest input list. For any input lists that are longer than the shortest one, the extra elements are ignored. If using `mapply()`, the function is applied to the elements of the longest input list and the shorter input lists are recycled to match the length of the longest input list. If one of the input lists is longer than the others and the length is not a multiple of the lengths of the other input lists, `mapply()` will return an error.

Thus, **when the input lists are of different lengths, maybe the best practice is to use `lapply()` to each of the lists and then combine them with `append()` or with `bind_rows()` if the output is a list of data frames.** See the example below:
...
One big drawback of `lapply()` is its long computation time, especially when the list length is long. Fortunately, parallel processing can help make it much faster.
...
In practice, we can split `mylist` into smaller chunks and apply the function to each element of the several chunks in parallel using functions under the `parallel` and the `future` packages to significantly reduce the run time.
