...
Within certain tidyverse operations, columns are called as if they were variables. For example, while running `r r df %>% mutate(X = …)` R recognizes that `r X` specifically references a column in `r df` without explicitly stating its membership [`r df %>% mutate(df$X = …)`] or calling the column name as a string [`r df %>% mutate(“X” = …)`] . 
