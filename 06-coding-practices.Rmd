...
Every function you write must include a header to document its purpose, inputs, and outputs. For any reproducible workflows they are essential because R is dynamically typed. This means you can pass a `string` into an argument that is meant to be a `data.table`, or a `list` into an argument meant for a `tibble`. It is the responsibility of a function's author to document what each argument is meant to do and its basic type.

We use roxygen2-style documentation headers (`#'` comments with `@param`, `@return` tags). This is the standard convention used in R package development, which means students will encounter it throughout the R ecosystem. It is also structured enough to be parsed programmatically, which supports automated code review. Here is an example:
...
## Tidyverse
R has two broad dialects you'll encounter: base R, which is the syntax built into the language, and the tidyverse, a collection of packages (including `dplyr`, `tidyr`, `ggplot2`, and `readr`) that provide an alternative, more consistent syntax for data manipulation and visualization. If you learned R in a short course, you may have used a mix of both without realizing it. Base R uses syntax like `df[df$age < 5, ]` and `merge()`, while tidyverse uses pipes and verbs like `df %>% filter(age < 5)` and `left_join()`. 

Both work, but our lab standardizes on tidyverse because the syntax is more readable and consistent across operations, which makes code easier to review and reproduce. The table below maps common base R patterns to their tidyverse equivalents for reference. For large data pipelines, `data.table` is a performant alternative. If you encounter base R patterns in legacy code or external packages, the reference table below maps common translations. Hadley Wickham's [R for Data Science](https://r4ds.had.co.nz/) is a good resource for learning the Tidyverse ecosystem.
...
Some projects may require both R and Python — for example, using Python for machine learning (e.g., `scikit-learn`, `pytorch`) and R for statistical analysis and visualization. There are two main strategies for interoperability:

**Exchanging data via Apache Arrow / Parquet.** The [Arrow](https://arrow.apache.org/docs/r/) and [Parquet](https://parquet.apache.org/) formats allow fast, zero-copy data exchange between R and Python with no loss of type information. In R, use the `arrow` package to read and write Parquet files:
...
**Calling Python from R with `reticulate`.** The [`reticulate`](https://rstudio.github.io/reticulate/) package lets you call Python functions directly from R, which can be useful when you want to use a Python library without leaving your R workflow. However, this adds complexity to reproducible environments (you need to manage both `renv` and a Python virtual environment), so use it judiciously.

**Note**: The `feather` package, which was previously recommended for R-Python data exchange, has been superseded by Arrow/Parquet and should no longer be used.
...
- Are functions documented with headers (see [Function documentation] above)?
- Does the code follow our style conventions (snake_case, named arguments, tidyverse)?

**Data integrity**

- Are assertions or sanity checks present where they should be (see the [Testing and validation] chapter)?
- Is the code careful about missing data — e.g., does it use `na.rm = TRUE` intentionally vs. ignoring missingness by default?
...
Be specific and constructive. Instead of "this looks wrong," say "the left join on line 45 uses `household_id` but I think it should use `individual_id` since we want one row per person." When something is a suggestion rather than a required change, label it as such (e.g., "Optional: you could simplify this with `across()`"). 
...
Describe how you tested the code:
- [ ] Script runs end-to-end without errors
- [ ] Output sample sizes match expectations
- [ ] Key estimates are plausible
- [ ] Assertions/sanity checks pass
