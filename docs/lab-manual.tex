...
\author{}
\date{\vspace{-2.5em}Updated: 2021-11-09}
...
To keep the install packages consistent across diferent nodes, you will need to explcitly

Open your \texttt{\textasciitilde{}/.Renviron} file (\texttt{vi\ \textasciitilde{}/.Renviron}) and append the following line:

\begin{Shaded}
\begin{Highlighting}[]
\VariableTok{R\_LIBS=}\NormalTok{\textasciitilde{}/R/x86\_64{-}pc{-}linux{-}gnu{-}library/4.0}
\end{Highlighting}
\end{Shaded}
...
\FunctionTok{scp} \AttributeTok{{-}r} \StringTok{"Box/malaria{-}project/folder{-}1/"}\NormalTok{ USERNAME@login.sherlock.stanford.edu:}
\FunctionTok{scp} \AttributeTok{{-}r} \StringTok{"Box/malaria{-}project/folder{-}2/"}\NormalTok{ USERNAME@login.sherlock.stanford.edu:}

\CommentTok{\# log into sherlock}
\FunctionTok{ssh}\NormalTok{ USERNAME@login.sherlock.stanford.edu}

\CommentTok{\# make a malaria{-}project subfolder in scratch directory}
\BuiltInTok{cd} \VariableTok{$SCRATCH}
\FunctionTok{mkdir}\NormalTok{ malaria{-}project}

\CommentTok{\# move files from sherlock home directory to scratch directory}
\BuiltInTok{cd} \VariableTok{$HOME}
\FunctionTok{mv}\NormalTok{ folder{-}1 }\VariableTok{$SCRATCH}\NormalTok{/malaria{-}project/}
\FunctionTok{mv}\NormalTok{ folder{-}2 }\VariableTok{$SCRATCH}\NormalTok{/malaria{-}project/}
\end{Highlighting}
\end{Shaded}
